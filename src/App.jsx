...
const App = () => {
  ...
  // ตรวจจับมือถือเพื่อสลับเป็น Week View ในครั้งแรก
  useEffect(() => {
    const checkMobile = () => {
  ...
  const getLocationStyle = (loc) => {
    switch (loc) {
      case 'ทำงานที่ออฟฟิศ': return 'bg-blue-50 text-blue-600 border-blue-100';
      case 'ทำงานที่บ้าน': return 'bg-emerald-50 text-emerald-600 border-emerald-100';
      case 'ทำงานนอกสถานที่': return 'bg-purple-50 text-purple-600 border-purple-100';
  ...
  const getLocationIcon = (loc) => {
    switch (loc) {
      case 'ทำงานที่ออฟฟิศ': return <Building2 size={12} />;
      case 'ทำงานที่บ้าน': return <Home size={12} />;
      case 'ทำงานนอกสถานที่': return <Globe size={12} />;
  ...
  const generateInitialData = () => {
    ...
    for (let day = 1; day <= 31; day++) {
      ...
      meta[dateKey] = { location: day % 3 === 0 ? 'ทำงานที่ออฟฟิศ' : day % 3 === 1 ? 'ทำงานที่บ้าน' : 'ทำงานนอกสถานที่' };

      if (day === 2 || day === 16) {
        leaves[dateKey] = [{ id: `l-${day}`, type: 'ลาพักร้อน', detail: 'พักผ่อนประจำปี', startTime: '09:00', endTime: '18:00', isFullDay: true }];
        entries[dateKey] = [];
        meta[dateKey].location = 'ทำงานที่บ้าน'; // กฎใหม่: ลาทั้งวัน = ทำงานที่บ้าน
        continue;
      }

      entries[dateKey] = [
        { id: `m1-${day}`, date: dateKey, jobNo: jobPool[0].id, jobName: jobPool[0].name, startTime: '09:00', endTime: '12:00', hours: 3, minutes: 0, detail: 'Morning Sync' },
        { id: `m2-${day}`, date: dateKey, jobNo: jobPool[1].id, jobName: jobPool[1].name, startTime: '13:00', endTime: '18:00', hours: 5, minutes: 0, detail: 'Development' }
      ];
    }
  ...
  const [newLeave, setNewLeave] = useState({
    date: '2026-03-09', type: 'ลาพักร้อน', detail: '', isFullDay: true, startTime: '09:00', endTime: '18:00'
  });
  ...
  const handleSaveEntry = () => {
    if (entryType === 'task') {
      ...
      setLeaveEntries(prev => {
        ...
        if (isMultiDateMode) {
          Object.entries(multiDateConfigs).forEach(([dKey, config]) => {
            newState[dKey] = [...(newState[dKey] || []), { ...newLeave, id: Date.now() + Math.random(), date: dKey, startTime: config.startTime, endTime: config.endTime }];
            
            if (newLeave.isFullDay) {
              setAllEntries(old => ({ ...old, [dKey]: [] }));
              newMeta[dKey] = { ...newMeta[dKey], location: 'ทำงานที่บ้าน' }; // Auto-select Home
            }
          });
        } else {
          const targetKey = newLeave.date;
          newState[targetKey] = [...(newState[targetKey] || []), { ...newLeave, id: Date.now() + Math.random() }];
          
          if (newLeave.isFullDay) {
            setAllEntries(old => ({ ...old, [targetKey]: [] }));
            newMeta[targetKey] = { ...newMeta[targetKey], location: 'ทำงานที่บ้าน' }; // Auto-select Home
          }
        }
  ...
  const openModal = (entry = null, date = null, type = 'task') => {
    ...
    if (entry) {
      ...
      setNewLeave({ date: targetDateKey, type: 'ลาพักร้อน', detail: '', isFullDay: true, startTime: '09:00', endTime: '18:00' });
  ...
  const ActiveHeaderBar = () => {
    ...
    return (
      <div className="bg-white/70 backdrop-blur-xl border-b border-slate-200 px-4 py-2.5 sticky top-[56px] sm:top-[64px] z-40">
          <div className="max-w-4xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3">
              <div className="flex items-center bg-slate-100 p-0.5 rounded-xl w-full sm:w-auto shadow-inner">
                  <button onClick={() => {
                  ...
                  <div className="flex-1 px-4 text-center cursor-pointer font-black text-[11px] text-slate-800 uppercase tracking-tight" onClick={() => setCurrentDate(new Date())}>
                      {viewMode === 'week' ? (
                          ...
                          currentDate.toLocaleDateString('th-TH', { month: 'short', year: 'numeric', day: viewMode === 'day' ? 'numeric' : undefined })
                      )}
                  </div>
                  <button onClick={() => {
                    const d = new Date(currentDate);
                    ...
                    setCurrentDate(d);
                  }} className="p-1.5 hover:bg-white rounded-lg text-slate-400"><ChevronRight size={16}/></button>
              </div>

              <div className="flex bg-slate-200/50 p-1 rounded-[14px] w-full sm:w-auto sm:min-w-[300px]">
                  {[
                    { id: 'ทำงานที่ออฟฟิศ', icon: Building2, label: 'ออฟฟิศ' },
                    { id: 'ทำงานที่บ้าน', icon: Home, label: 'บ้าน' },
                    { id: 'ทำงานนอกสถานที่', icon: Globe, label: 'Field' }
                  ].map(loc => (
                    <button 
                      key={loc.id}
                      onClick={() => {
                        const newMeta = { ...dayMetadata, [currentDateKey]: { ...dayMetadata[currentDateKey], location: loc.id } };
                        setDayMetadata(newMeta);
                      }}
                      className={`flex-1 flex flex-col items-center justify-center py-1.5 rounded-xl transition-all ${currentLocation === loc.id ? 'bg-white text-blue-600 shadow-sm scale-[1.02]' : 'text-slate-400'}`}
                    >
  ...
  const renderMonthView = () => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      const dayMeta = dayMetadata[key];

      cells.push(
        <div key={d} onClick={() => setCurrentDate(date)} onDoubleClick={() => openModal(null, date)}
          className={`group flex flex-col min-h-[85px] border-r border-b border-slate-100 p-1 cursor-pointer transition-all ${isSelected ? 'bg-blue-50/60 ring-2 ring-inset ring-blue-200' : 'bg-white'}`}
        >
          <div className="flex justify-between items-start">
            ...
            {dayMeta && <div className="text-slate-200 transition-colors">{getLocationIcon(dayMeta.location)}</div>}
          </div>
    ...
    return (
      <div className="flex flex-col w-full bg-white pb-32 animate-in fade-in duration-300">
        <ActiveHeaderBar />
        <div className="grid grid-cols-7 bg-slate-50 border-b border-slate-200 sticky top-[112px] sm:top-[120px] z-20 shadow-sm">
        ...
        <div className="p-6 pb-20 max-w-2xl mx-auto w-full border-t border-slate-50">
            <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest mb-4 flex items-center gap-2"><ListTodo size={16}/> Daily Summary</h3>
            <div className="space-y-2">
                {currentDayLeaves.map(l => (
                  <div key={l.id} className="bg-orange-50/50 p-4 rounded-3xl flex items-center gap-4 border border-orange-100">
                    ...
                    <div className="flex-1 min-w-0"><h4 className="text-sm font-black text-slate-800">{l.type}</h4><p className="text-[10px] text-orange-400 font-bold">{l.startTime} - {l.endTime}</p></div>
                  </div>
                ))}
                {currentDayEntries.map(e => (
                  <div key={e.id} onClick={() => openModal(e)} className="bg-white p-4 rounded-2xl flex items-center gap-4 border border-slate-100 shadow-sm active:scale-[0.98] transition-all">
                    ...
                    <div className="flex-1 min-w-0"><p className="text-[8px] font-black text-blue-500 uppercase">#{e.jobNo}</p><h4 className="text-sm font-black text-slate-800 truncate">{e.jobName}</h4></div>
  ...
  const renderWeekView = () => {
    ...
    return (
      <div className="flex flex-col w-full bg-slate-50 min-h-screen pb-40 animate-in slide-in-from-right-4 duration-500">
        <ActiveHeaderBar />
            ...
            {weekDays.map(d => {
                ...
                return (
                  <div key={k} 
                       ...
                       onDoubleClick={() => openModal(null, d)}
                       className={`bg-white rounded-[32px] p-5 border-2 transition-all active:scale-[0.98] w-full ${isSelected ? 'border-blue-500 shadow-xl' : 'border-transparent shadow-sm'}`}>
                    <div className="flex justify-between items-center mb-4">
                        <div className="flex items-center gap-3">
                            <span className={`text-2xl font-black ${isSelected ? 'text-blue-600' : 'text-slate-800'}`}>{d.getDate()}</span>
                            <span className="text-[11px] font-black text-slate-400 uppercase tracking-widest">{d.toLocaleDateString('en-US', { weekday: 'short' })}</span>
                        </div>
                        {loc && (
                            <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full border text-[8px] font-black uppercase tracking-tighter ${getLocationStyle(loc)}`}>
                                {getLocationIcon(loc)} {loc.replace('ทำงานที่', '')}
                            </div>
                        )}
                    </div>
                    <div className="space-y-1.5">
                        {leaves.map(l => <div key={l.id} className="text-[10px] font-black text-orange-600 bg-orange-50 px-3 py-2 rounded-2xl border border-orange-100 flex items-center gap-2 shadow-sm"><UserX size={12}/> {l.type}</div>)}
  ...
  const renderDayView = () => {
    const totalH = currentDayEntries.reduce((sum, e) => {
        const d = calculateDuration(e.startTime, e.endTime);
        return sum + d.h + (d.m/60);
    }, 0);
          ...
          <div className="max-w-lg mx-auto w-full p-6">
              ...
              <div className="space-y-4">
                  ...
                  <div className="pt-4 flex justify-center">
                      <span className="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em]">{totalH.toFixed(1)} Working Hours Today</span>
                  </div>
              </div>
          </div>
      </div>
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 transition-all selection:bg-blue-100 overflow-x-hidden">
      <header className="fixed top-0 left-0 right-0 z-[100] bg-white/70 backdrop-blur-2xl border-b border-slate-200 px-6 h-[56px] sm:h-[64px] flex justify-between items-center">
        <div className="flex items-center gap-2">
            ...
            <h1 className="text-base font-black italic tracking-tighter uppercase text-slate-900 select-none">TimeFlow</h1>
        </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[500] flex items-end sm:items-center justify-center p-0 sm:p-6 animate-in fade-in duration-300">
           <div className="bg-white w-full max-w-lg rounded-t-[40px] sm:rounded-[32px] shadow-2xl overflow-hidden animate-in slide-in-from-bottom duration-300 flex flex-col max-h-[92vh]">
              ...
              <div className="h-14 bg-slate-50 border-b border-slate-200 flex items-center justify-between px-8">
                <div className="flex items-center gap-3 text-blue-600">
                    {entryType === 'leave' ? <UserX size={20}/> : <Plus size={20}/>}
                    <h2 className="text-xs font-black uppercase tracking-widest">{editingId ? 'Edit Record' : 'Create Entry'}</h2>
                </div>
              ...
              <div className="p-6 overflow-y-auto scrollbar-thin">
                 <div className="flex bg-slate-100 p-1 rounded-xl mb-6 shadow-inner">
                    <button onClick={() => setEntryType('task')} className={`flex-1 py-2.5 text-[10px] font-black uppercase rounded-xl transition-all ${entryType === 'task' ? 'bg-white shadow-md text-blue-600' : 'text-slate-400'}`}>Task / Job</button>
                    <button onClick={() => setEntryType('leave')} className={`flex-1 py-2.5 text-[10px] font-black uppercase rounded-xl transition-all ${entryType === 'leave' ? 'bg-white shadow-md text-orange-600' : 'text-slate-400'}`}>Leave Request</button>
                 </div>
                 ...
                 <div className="space-y-6">
                    {isMultiDateMode ? (
                        <div className="space-y-4">
                            <div className="grid grid-cols-7 gap-1 p-3 bg-slate-50 rounded-2xl border border-slate-100 shadow-inner">
                                {[...Array(31)].map((_, i) => {
                                    ...
                                    return <button key={i} onClick={() => toggleRepeatDate(dK)} className={`w-7 h-7 rounded-xl text-[9px] font-black flex items-center justify-center transition-all ${!!multiDateConfigs[dK] ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-300 hover:bg-slate-200'}`}>{i+1}</button>
                                })}
                            </div>
                            <div className="space-y-2">
                                {Object.entries(multiDateConfigs).sort().map(([dk, c]) => (
                                    <div key={dk} className="bg-slate-50 p-4 rounded-3xl border border-slate-100 flex flex-col gap-3 animate-in slide-in-from-right-4">
                                        <div className="flex justify-between items-center">
                                            ...
                                            {(entryType === 'leave' && newLeave.isFullDay) ? <span className="text-[8px] font-black bg-orange-100 text-orange-600 px-2 rounded">Full Day Leave</span> : null}
                                        </div>
                                        {!(entryType === 'leave' && newLeave.isFullDay) && (
                                            <div className="grid grid-cols-2 gap-3 flex-1">
                                                <input type="time" value={c.startTime} onChange={e => updateMultiTime(dk, 'startTime', e.target.value)} className="bg-white border rounded-xl px-2 py-2 text-xs font-black outline-none focus:border-blue-500"/>
                                                <input type="time" value={c.endTime} onChange={e => updateMultiTime(dk, 'endTime', e.target.value)} className="bg-white border rounded-xl px-2 py-2 text-xs font-black outline-none focus:border-blue-500"/>
                                            </div>
                                        )}
                                    </div>
                                ))}
                        ...
                        <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Date</label><input type="date" value={entryType === 'task' ? newTask.date : newLeave.date} onChange={e => entryType === 'task' ? setNewTask({...newTask, date: e.target.value}) : setNewLeave({...newLeave, date: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-3.5 text-sm font-bold focus:bg-white outline-none transition-all shadow-inner"/></div>
                    )}

                    {entryType === 'task' ? (
                        <div className="space-y-6">
                            <div className="relative space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase ml-2">Project ID</label><div className="relative"><input type="text" value={newTask.jobNo} onChange={e => setNewTask({...newTask, jobNo: e.target.value})} placeholder="69XXX" className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-3.5 text-sm font-bold outline-none pr-10 shadow-inner"/><Search className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-300" size={16}/></div>
                                {suggestions.length > 0 && (<div className="absolute z-[600] w-full bg-white border border-slate-200 rounded-2xl mt-1 shadow-xl max-h-40 overflow-y-auto">{suggestions.map((job, idx) => (<button key={idx} onClick={() => selectJob(job)} className="w-full text-left px-5 py-3 text-[10px] hover:bg-blue-50 border-b last:border-0 font-black flex justify-between"><span>{job.id}</span><span className="text-slate-400">{job.name}</span></button>))}</div>)}
                            </div>
                            ...
                            {!isMultiDateMode && (
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase ml-2">Start</label><input type="time" value={newTask.startTime} onChange={e => setNewTask({...newTask, startTime: e.target.value})} className="w-full bg-white border-2 border-slate-200 rounded-2xl px-4 py-3 text-lg font-black text-slate-800 focus:border-blue-500 outline-none"/></div>
                                    <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase ml-2">End</label><input type="time" value={newTask.endTime} onChange={e => setNewTask({...newTask, endTime: e.target.value})} className="w-full bg-white border-2 border-slate-200 rounded-2xl px-4 py-3 text-lg font-black text-slate-800 focus:border-blue-500 outline-none"/></div>
                                </div>
                            )}
                            <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase ml-2">Notes</label><textarea value={newTask.detail} onChange={e => setNewTask({...newTask, detail: e.target.value})} rows="3" placeholder="Additional details..." className="w-full bg-slate-100/50 border border-slate-200 rounded-[28px] px-6 py-4 text-sm font-bold outline-none resize-none focus:bg-white transition-all"></textarea></div>
                        </div>
                    ) : (
                        <div className="space-y-6">
                            ...
                            <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100 shadow-inner"><input type="checkbox" id="ios-fday" checked={newLeave.isFullDay} onChange={e => setNewLeave({...newLeave, isFullDay: e.target.checked})} className="w-6 h-6 rounded-lg text-orange-600 border-slate-300"/><label htmlFor="ios-fday" className="text-xs font-black text-slate-700 uppercase tracking-widest cursor-pointer">ลาทั้งวัน (Full Day Leave)</label></div>
                            {(!isMultiDateMode && !newLeave.isFullDay) && (
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase ml-2">เริ่มเวลา</label><input type="time" value={newLeave.startTime} onChange={e => setNewLeave({...newLeave, startTime: e.target.value})} className="w-full bg-white border-2 border-orange-200 rounded-2xl px-4 py-3 text-lg font-black text-slate-800 outline-none"/></div>
                                    <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase ml-2">สิ้นสุดเวลา</label><input type="time" value={newLeave.endTime} onChange={e => setNewLeave({...newLeave, endTime: e.target.value})} className="w-full bg-white border-2 border-orange-200 rounded-2xl px-4 py-3 text-lg font-black text-slate-800 outline-none"/></div>
                                </div>
                            )}
                            <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase ml-2">เหตุผลการลา</label><textarea value={newLeave.detail} onChange={e => setNewLeave({...newLeave, detail: e.target.value})} placeholder="ระบุเหตุผล..." rows="3" className="w-full bg-slate-100/50 border border-slate-200 rounded-[28px] px-6 py-4 text-sm font-bold outline-none resize-none focus:bg-white"></textarea></div>
                        </div>
                    )}
                    <button onClick={handleSaveEntry} className={`w-full py-5 text-white rounded-[24px] text-[11px] font-black uppercase shadow-xl active:scale-95 transition-all ${entryType === 'task' ? 'bg-blue-600' : 'bg-orange-600 shadow-orange-100'}`}>Save Record</button>
                 </div>
              </div>
           </div>
        </div>
